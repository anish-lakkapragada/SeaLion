'''
# TODO NUMPY SLOWS EVERYTHING DOWN IN CYTHON, WHAT IF I JUST RETURN A PYTHON LIST AND THEN DO NP.ARRAY(LIST) IN WRAPPER CLASS?
'''
...
#todo this is also a backbone method along with distance()
cdef list find_neighbors(list data, list point, double eps):
    '''neighbors = []
    cdef list data_point
    for data_point in data:
        if data_point == point: continue
        if distance(data_point, point) <= eps:
...
#TODO RECURSION METHOD TAKES THE MOST COMPUTATION
...
#todo have fun with the DBSCAN optimization here
cdef class CythonDBSCAN():
    cdef double eps
    cdef dict clusters, points_to_classes, visualize_clusters

    #ODO REALIZE X_TRAIN COULD BE NUMPY ARRAY - > FIGURE OUT WHETHER CYTHON CARES
    cpdef list fit_predict(self, x_train,  double eps=0.5, int min_neighbors=7):
        ...
        cdef dict predictions_dict = {} #todo this is the problem with repeats in the core instances!
    ...
    #TODO INTERGRATE VIZ INTO PYTHON WRAPPER CLASS
    '''def visualize_clustering(self, colors_dict):
        ...
        for point, label in self.points_to_classes.items() :
            point_and_class_list.append([list(point), label])

        self.classes_to_points = defaultdict(list)
        for point_and_class in point_and_class_list :
            point, label= point_and_class[0], point_and_class[1]
            self.classes_to_points[label].append(point)

        for label, points in self.classes_to_points.items() :
            plt.scatter(np.array(points)[:, 0], np.array(points)[:, 1], color=colors_dict[label], label = "cluster : " + str(label))
        plt.legend()
...
cdef class CythonKMeans():
    ...
    #TODO INTEGRATE ALL VIZ INTO PYTHON WRAPPER
    '''def visualize_elbow_curve(self, min_k = 1, max_k = 10) :
        inertia_scores = []
        max_k += 1
        for k_value in range(min_k, max_k) :
            KMeans.fit_predict(self, self.data, k = k_value)
            inertia_scores.append(KMeans.inertia_score(self))
