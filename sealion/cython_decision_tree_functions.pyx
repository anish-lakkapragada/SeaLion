...
'''cpdef dict build_tree(list data, list labels, int min_samples, double max_branches = math.inf):
        try :
            data = np.array(data).tolist()
            labels = np.array(labels).tolist()
        except Exception :
            pass
        best_split = find_best_split(data, labels, self.min_samples)
        ...
        try:
            ...
            tree = {tuplelize(best_split): {True: DecisionTree.build_tree(self, true_branch, true_labels, max_branches),
                                            False: DecisionTree.build_tree(self, false_branch, false_labels, max_branches)}}
            return tree'''
