...
from sealion.neural_networks.layers import Dropout, Softmax, Dense, Flatten
from sealion.neural_networks.loss import CrossEntropy, softmax
from sealion.neural_networks.optimizers import SGD, Adam
import numpy as np
import warnings
...
import random
from sealion.utils import revert_softmax, revert_one_hot
...
class NeuralNetwork() :
    ...
    >>> from sealion.utils import revert_softmax
    ...
    >>> model.pickle_weights(WEIGHT_FILE_NAME=file_name)
    ...
    def pickle_weights(self, WEIGHT_FILE_NAME = "NeuralNetwork_learnt_parameters"):
        parameters = NeuralNetwork.give_parameters(self)
        import pickle

        with open(WEIGHT_FILE_NAME + '.pickle', 'wb') as f:
    ...
    def forward(self, inputs):
        for layer in self.layers :
            if isinstance(layer, Softmax) : continue
    ...
    def train(self, x_train, y_train, epochs=1, batch_size=32, show_loop = True):
        ...
        if isinstance(self.layers[-1], Softmax) and not isinstance(self.loss, CrossEntropy) :
            raise ValueError("If the last layer activation is softmax, you must be using CrossEntropy loss.")

        if isinstance(self.loss, CrossEntropy) and not isinstance(self.layers[-1], Softmax) :
        ...
        if not isinstance(self.layers[0], Flatten) and len(x_train.shape) > 2:
    ...
    def predict(self, x_test):
        ...
        if isinstance(self.layers[-1], Softmax) : y_pred = softmax(y_pred)
    ...
    def categorical_evaluate(self, x_test, y_test) :
        ...
        return _perc_correct(revert_softmax(y_pred).flatten(), revert_one_hot(y_test).flatten())
...
class NeuralNetwork_MapReduce():
    ...
    You could also just get the NeuralNetwork() class of the best model through :
    >>> nn_model = model.get_best_model(X_test, y_test)
    >>> print(nn_model)
    <models.NeuralNetwork object at 0xsomethingsomething>

    and use that. There's another method of give_best_parameters() which is also very similar to the pickle_best_parameters().
    ...
    def pickle_best_parameters(self, x_test, y_test, WEIGHT_FILE_NAME =  "NeuralNetwork_learnt_parameters"):
        if len(y_test.shape) != 2: raise ValueError("y_test must be 2D.")
    ...
    def categorical_evaluate(self, x_test, y_test) :
        ...
        return _perc_correct(revert_softmax(y_pred), revert_one_hot(y_test))
