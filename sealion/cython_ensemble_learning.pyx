...
cdef class CythonEnsembleClassifier() :
    ...
    cpdef np.ndarray predict(self, x_test) :
        ...
        cdef np.ndarray[np.float32_t, ndim = 1] predictions = np.zeros(len(x_test)).astype(np.float32)
    ...
    cpdef double evaluate(self, x_test, y_test) :
        ...
        cdef np.ndarray[np.float32_t, ndim = 1] y_pred = CythonEnsembleClassifier.predict(self, x_test)
